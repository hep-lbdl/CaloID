...
from models import (train_caloclf_model, build_lagan_style_model,
                    build_shower_shape_model, build_densenet_model,
                    build_raveled_model)
...
if __name__ == '__main__':
    ...
    logger.info('Reading from basedir = {}, ending = {}'.format(BASEDIR, ENDING))
    ...
    for job_id, job in enumerate(cfg['jobs']):
        ...
        training_hparams = {k: safe_to_list(v)
                            for k, v in job['training'].items()}
        ...
        for count, (t_hparams, m_hparams) in enumerate(hparam_grid):
            logger.info('performing grid element {} of {}'
                        .format(count + 1, nb_selections))
